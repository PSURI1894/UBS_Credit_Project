...
# Example rows (replace later with your actual cluster_profile.xlsx)
example_data = [
...
# UBS Recommendations mapping table
recommendations = {
    "Premier": [
...
# ==============================
# Sheet 3: Auto-Lookup View
# ==============================
ws3 = wb.create_sheet("Cluster + Actions")

ws3.append(["Cluster", "Suggested Segment", "Auto-Linked Recommendations"])

# Create formulas using VLOOKUP to auto-link
# Assuming "Cluster Mapping" is sheet1 and "Segment Recommendations" is sheet2
for i in range(2, 6):  # for 4 clusters
    # Link cluster ID
    ws3.cell(row=i, column=1, value=f"=INDIRECT(\"'Cluster Mapping'!A{i}\")")
    # Link suggested segment
    ws3.cell(row=i, column=2, value=f"=INDIRECT(\"'Cluster Mapping'!I{i}\")")
    # Lookup recommendation using VLOOKUP
    ws3.cell(row=i, column=3, value=f"=VLOOKUP(B{i},'Segment Recommendations'!A:B,2,FALSE)")

# Style header
for col in range(1, 4):
    ws3.cell(row=1, column=col).font = Font(bold=True, color="FFFFFF")
    ws3.cell(row=1, column=col).fill = PatternFill(start_color="F79646", end_color="F79646", fill_type="solid")

# Save workbook
wb.save("ubs_cluster_mapping_auto.xlsx")
print("File saved as ubs_cluster_mapping_auto.xlsx")
